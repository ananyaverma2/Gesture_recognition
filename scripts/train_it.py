...
class mpHands:
    ...
    def __init__(self, mode=False, maxHands=2,modelC=1, detectionCon=0.5, trackCon=0.5):
        ...
        self.hands=self.mp.solutions.hands.Hands(False,maxHands,modelC,detectionCon,trackCon)
...
while True:
    ...
    if train==1:
        if handData!=[]:
            print('Please Show Gesture ',gestNames[trainCnt],': Press Esc when Ready')
            # pdb.set_trace()
            # k = int(input("enter 1 to go to the next one"))
            if pyautogui.press('shift'):
                print(" inside")
